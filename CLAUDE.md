...
- **immich.py** - Standalone Immich integration script called by `bin/epm` after extraction. Triggers a library scan, polls for new assets, creates/reuses an album named after the video, adds assets, optionally shares the album, and optionally sends a Pushover notification. Uses only stdlib (`urllib.request`, `urllib.parse`, `json`). Can be run directly: `python extract_photos/immich.py --api-url ... --api-key ... --library-id ... --asset-path ... --video-filename ...`.
